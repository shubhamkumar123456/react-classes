...
const Cart = () => {
  ...
  return (
    <div>
     ...
     { cartItem.length >0 ? <table className=' w-[85%] text-center mx-auto text-white'>
        <thead>
     ...
     { cartItem.length>0 && <h1 className='text-4xl font-semibold w-max m-auto my-5'>Total = ${sum.toFixed(2)}</h1>}

      { cartItem.length >0 && <button className='bg-green-900 text-white py-2 px-4 rounded-md hover:bg-green-600 w-max m-auto block'>Buy Now</button>}
    </div>
  )
}
